...
// Draw all the shapes, first rotations by 45, then scaled by 1.5, 1.0x
void doRotateThenScale() {
	dmat3 TM = S(1.5, 1.0) * R(deg2rad(45));
	drawObjectAndAllTransformations(TM);
}

// Draw all the shapes, first scale by 1.5, 1.0X, then rotation by 45
void doScaleThenRotate() {
	dmat3 TM = R(deg2rad(45)) * S(1.5, 1.0);
	drawObjectAndAllTransformations(TM);
}

// Draw only triangle, scaled 2X about its center (-1.5*N, 2.5*N)
void doScale2XAboutCenterOfTriangle() {
	dmat3 TM = T(-1.5*N, 2.5*N) * S(2.0, 2.0) * T(1.5*N, -2.5*N);
	drawObjAndOneTransformation(TM, triangleVertices);
}

// Draw all shapes, reflected across y=x+50
void doReflectAcrossLineYeqXplus50() {
	dmat3 TM = T(0.0, 50.0) * R(deg2rad(45.0)) * S(1.0, -1.0) * R(deg2rad(-45.0)) * T(0.0,-50.0);
	drawObjectAndAllTransformations(TM);
}
...
void doSquareRotatingAroundOwnAxisAndAroundSun() {
	...
	double angleInRads = deg2rad(angleInDegs);
	dmat3 TTO = T(200.0 * glm::cos(angleInRads), 200.0 * glm::sin(angleInRads));
	dmat3 TM = TTO * R(deg2rad(4.0 * angleInDegs));
